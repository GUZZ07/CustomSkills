			...
			foreach (var data in Data.ProjDatas)
			{
				void apply()
				{
					...
					else
					{
						...
						plugin.Add(new ProjShiftOperation(proj, data.Velocity, data.LaunchDelay));
					}
					foreach (var shift in data.Shifts)
					{
						plugin.Add(new ProjShiftOperation(proj, shift.Velocity, data.LaunchDelay + shift.Delay));
					}
				}
